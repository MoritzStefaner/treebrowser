package eu.stefaner.organicedunet.semanticsearch.treeexplore_vertical {
	 ...
	 */
	public class OEVisibilityFilter extends Operator {
		...
		public var breadcrumbNodes : DataList;
		public var relatedNodes : DataList;
		public var childNodes : DataList;
		 ...
		 */		
		public function OEVisibilityFilter(focusNodes : Vector.<Object> = null,distance : int = 1) {
		...
		public override function operate(t : Transitioner = null) : void {
			...
			if(tree.group("breadcrumbNodes") == null) {
				initGroups(tree);
			}
		...
		private function showRelatedItems() : void {
			for each (var ns:NodeSprite in focusNodes) {
				ns.visitEdges(function(e : EdgeSprite) {
					setVisibility(e, true);
					var ns2 : NodeSprite = e.other(ns);
					setVisibility(ns2, true);
					if(!relatedNodes.contains(ns2)) {
						Logger.info("adding", ns2.data.label);
						relatedNodes.add(ns2);
					}
		...
		private function processGroups() : void {
			for each (var ns:NodeSprite in focusNodes) {
				...
				ns.visitNodes(function(ns2 : NodeSprite) {
					childNodes.add(ns2);
				}, NodeSprite.CHILD_LINKS);
				
				breadcrumbNodes.add(ns);
				var p : NodeSprite = ns;
				while (p = p.parentNode) {
					breadcrumbNodes.add(p);
				}
			}
		}

		private function emptyGroups(tree : Tree) : void {
			childNodes.clear();
			relatedNodes.clear();
			breadcrumbNodes.clear();
		}

		private function initGroups(tree : Tree) : void {
			childNodes = tree.addGroup("childNodes");
			relatedNodes = tree.addGroup("relatedNodes");
			breadcrumbNodes = tree.addGroup("breadcrumbNodes");
		}
